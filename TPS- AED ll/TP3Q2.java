...
//---------------------------------------------------------------------------------
class No2 {
	public PropriedadeArquivo elemento; // Conteudo do no2.
	public No2 esq; // No2 da esquerda.
	public No2 dir; // No2 da direita.
	
   /**
	 * Construtor da classe.
	 * @param elemento Conteudo do no2.
	 */
	No2(PropriedadeArquivo elemento) {
	 ...
	 */
	No2(PropriedadeArquivo elemento, No2 esq, No2 dir) {
		this.elemento = elemento;
		this.esq = esq;
		this.dir = dir;
	}
...
class ArvoreArvore {
	...
	private void mostrarCentral(No no) {
		...
		System.out.println(no.outro.elemento.getNome()+ " "); // Conteudo do no.
 ...
 public void mostrarPre() {
    ...
    private void mostrarPre(No no) {
        if (no != null) {
            System.out.print(no.outro.elemento.getNome() + " "); // Conteudo do no.
    ...
    private boolean pesquisar(No no, String x ) {
      ...
      } else if ((Integer.parseInt(x) %15) == (no.elemento) ) { 
...
/**
    * @param Propiedade arquivo
    * Metodo para inserir
    * na segunda arvore.
    * ( chave nome do arquivo )
    */
 
   void inserirDNV(PropriedadeArquivo x)throws Exception{

    raiz =  inserirDNV(raiz, x);

   }
  
     public No inserirDNV(No no, PropriedadeArquivo x)throws Exception{
	  
 	int mod = Integer.parseInt(x.getNome()) % 15 ; 
       
      	if (mod == no.elemento){  
        	no.outro = inserirSegundaArvore(no.outro, x); 
        }
        else if (mod < no.elemento){
        	no.esq = inserirDNV(no.esq, x); 
        }
        else if( mod > no.elemento ){ 
        	no.dir = inserirDNV(no.dir, x); 
        }
        else{
    ...
    public No2 inserirSegundaArvore ( No2 no, PropriedadeArquivo x )throws Exception{
      ...
      if (no == null) {
         ...
         no.esq = inserirSegundaArvore(no.esq, x);
 
      } else if (x.getNome().compareTo(no.elemento.getNome()) > 0) {
         no.dir = inserirSegundaArvore(no.dir, x);
   ...
   * de arvore elementos e usando o metodo
   * da classe arvoreArvore pesquisar
   */


    void questao2() throws Exception{
  	
      ArvoreArvore binaria = new  ArvoreArvore();
     
        for(String nomeArquivo = MyIO.readLine(); nomeArquivo.equals("FIM") == false; nomeArquivo =  		    MyIO.readLine()){
    
     	 binaria.inserirDNV(new PropriedadeArquivo(nomeArquivo));
