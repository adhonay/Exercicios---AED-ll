...
class HashDR {
   ...
   boolean inserir (Partida valor)throws Exception
   {
      boolean resp = false;
      int pos = funcaoTransformacao(valor.getProbMandante());  
      int x = funcaoTransformacao(valor.getProbMandante()+1);  
      if( tabela[pos].getProbMandante() == 0 ) 
      {
       tabela[pos] = valor;
       resp = true;
      }
      else if ( tabela[x].getProbMandante() == 0 ) 
      {
		tabela[x] = valor;
		resp = true;
       }
   ...
   int pesquisar(String valor1)
   {
    ...
    if(tabela[pos].getProbMandante() == x )
    {
	resp = pos;
     }
    else
